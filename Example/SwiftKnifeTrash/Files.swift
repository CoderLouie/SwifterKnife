...
//  SwifterKnife
//
//  Created by liyang on 2021/11/15.
//
...
public extension Location {
    ...
    func path(relativeTo folder: Folder) -> String {
        ...
        return String(path[index...])._removingSuffix("/")
    }
...
private extension Storage where LocationType == Folder {
    ...
    func subfolder(at folderPath: String) throws -> Folder {
        let folderPath = path + folderPath._removingPrefix("/")
    ...
    func file(at filePath: String) throws -> File {
        let filePath = path + filePath._removingPrefix("/")
...
public extension Folder {
    ...
    struct ChildIterator<Child: Location>: IteratorProtocol {
        ...
        public mutating func next() -> Child? {
            ...
            let childPath = folder.path + name._removingPrefix("/")
...
public extension Folder {
    ...
    static var home: Folder {
        return try! Folder(path: NSHomeDirectory())
    }
    ...
    /// The current user's Caches folder
    static var caches: Folder {
        return try! .matching(.cachesDirectory)
    }
    
    static var preference: Folder {
        return try! Folder(path: NSHomeDirectory() + "/Library/Preference")
    }
    ...
    static var bundle: Folder {
        return try! Folder(path: Bundle.main.bundlePath)
    }
}
...
private extension String {
    func _removingPrefix(_ prefix: String) -> String {
        guard hasPrefix(prefix) else { return self }
        return String(dropFirst(prefix.count))
    }

    func _removingSuffix(_ suffix: String) -> String {
